...
def get_contacts(campaign_id):
    result = supabase.table("contacts").select("*").eq("campaign_id", campaign_id).execute()
...
def bulk_create_contacts():
    ...
    rows = [{"campaign_id": campaign_id, "name": c.get("name", ""), "phone": c.get("phone", ""), "status": "pending"} for c in contacts]
...
def update_status(contact_id):
    data = request.json
    result = supabase.table("contacts").update({"status": data.get("status")}).eq("id", contact_id).execute()
